...
- Ruta secreta (`APP_SECRET_PATH` en `.env`, default `/`): ofuscación deliberada. Cuando NO es `/`, Caddy responde 404 a todo salvo `{$APP_SECRET_PATH}*`, `/api/*` y `/health`, y `vite.config.ts` pone `base` a esa ruta (build y dev server). Se pasa como env var a los servicios `frontend` y `caddy` en `docker-compose.yml`. El matcher `@app path {$APP_SECRET_PATH:/}*` con default `/` se vuelve `/*` (cubre todo → comportamiento original). No es un reemplazo del login/MFA; si se toca, reprobar `/`, la ruta secreta con y sin barra final, `/api/auth/me` y `/health`. Cambiar la ruta requiere `up -d --build frontend caddy` (el `base` de Vite se hornea en el bundle).
