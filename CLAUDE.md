...
- MFA (TOTP, RFC 6238) obligatorio desde el primer login: `POST /api/auth/login` ya no entrega cookie directamente — responde `{mfaSetupRequired, otpauthUrl, pendingToken}` (primera vez) o `{mfaRequired, pendingToken}` (ya configurado); `POST /api/auth/mfa/confirm` valida el código de 6 dígitos y recién ahí entrega la cookie de sesión. Implementado a mano con `hmac`+`hashlib.sha1` (sin librerías). `POST /api/users/{id}/reset-mfa` (Administrador) limpia `totp_secret`/`mfa_enabled`/`last_totp_step` de un usuario.
- Anti-replay de TOTP (RFC 6238 §5.2): `verify_totp` devuelve el paso (contador de 30 s) con el que el código valida, o `None`. `consume_totp(conn, user, code)` lo valida y persiste `users.last_totp_step`; rechaza cualquier código de un paso ya consumido (devuelve `"ok"`/`"reused"`/`"invalid"`). Lo usan tanto `api_mfa_confirm` (login) como `api_mfa_reauth`. Consecuencia: un código sirve una sola vez — si necesitas MFA dos veces en el mismo tramo de 30 s, la segunda espera el código siguiente (el backend responde 401 "Ese código ya se usó…"). Los tests resetean `last_totp_step` entre login/reauth (`_reset_totp_replay_guard`) porque corren varios dentro del mismo tramo.
...
- Ruta secreta (`APP_SECRET_PATH` en `.env`, default `/`): ofuscación deliberada. Cuando NO es `/`, Caddy responde 404 (disfrazado de nginx: `Server: nginx` + cuerpo de 404 de nginx, sin cabeceras de seguridad) a todo salvo `{$APP_SECRET_PATH}*`, `/api/*` y `/health`. `/api/*` y `/health` también uniforman su `Server` a `nginx` vía `header_down`. `vite.config.ts`: en `serve` (dev) `base` = `APP_SECRET_PATH` (Vite dev lo necesita); en `build` (prod) `base` = `'./'` (rutas de assets relativas) para que el bundle sirva bajo cualquier prefijo sin reconstruir — el proxy hace `handle_path` (strip del prefijo). En docker-compose el proxy NO hace strip (matchea la ruta tal cual, porque dev usa base absoluta); en OpenShift SÍ hace strip. No es un reemplazo del login/MFA; si se toca, reprobar `/` (404), la ruta secreta con y sin barra final, `/api/auth/me` y `/health`.
- Backup del storage de Vault (contraseñas de portal, separado del backup de SQLite): sidecar `vault-backup` en el pod de `vault` (mismo patrón que `backup_daemon.py` pero para `/vault/data`), `backend/scripts/vault_backup_daemon.py` + `restore_vault_backup.py`. Escribe en el PVC `securevault-vault-backups` (OpenShift) / `./vault-backups` (compose), separado de `securevault-vault-data` para no perder ambos ante el mismo fallo de disco. Como Vault con `storage "file"` no tiene una API de snapshot en caliente (a diferencia de `sqlite3.backup()`), el script simplemente empaqueta el directorio con `tarfile` mientras Vault sigue corriendo — riesgo bajo pero no nulo de capturar un archivo a medio escribir; la alternativa robusta sería migrar a `storage "raft"` y usar `vault operator raft snapshot save`, pendiente. Restaurar requiere Vault detenido (StatefulSet a 0 réplicas) y la `unseal_key` vigente al momento de ese backup específico — este backup no reemplaza la copia externa de `vault-keys.json`, son protecciones independientes.
- Migración opcional a `storage "raft"` (Integrated Storage): `openshift/11-vault-raft-migrate-job.yaml` + guía paso a paso en `openshift/README.md` ("Migración a Integrated Storage"). Usa `vault operator migrate` (herramienta oficial de Vault, no toca el backend en absoluto) para copiar de `securevault-vault-data` (file) a `securevault-vault-raft-data` (raft) sin tocar el PVC original. `01-config.yaml` ya trae la clave `vault-raft.hcl` lista en el ConfigMap `securevault-vault-config` junto a la `vault.hcl` actual — migrar es cuestión de reapuntar `04-vault.yaml` a esa clave y al PVC nuevo, no de escribir HCL nuevo a mano. No se hizo automático porque toca datos de producción (contraseñas de portal reales); queda documentado para correrlo cuando el usuario programe la ventana de mantenimiento.
- Despliegue en OpenShift: `openshift/*.yaml` numerados (00→10) + `openshift/README.md`. Vault como StatefulSet 1 réplica con sidecar `unsealer`; `vault/bootstrap/bootstrap.py` gana modo `KEYS_BACKEND=k8s-secret` (escribe las llaves en el Secret `securevault-vault-keys` vía la API de k8s con el token del SA en vez de un archivo). El proxy Caddy corre sin TLS (el Route hace edge TLS). El backup es un `CronJob` (`backup_daemon.py --once`, flag nuevo). Las 3 imágenes propias salen de `backend/Dockerfile` (multipropósito: API + CronJob + Job de bootstrap) y `frontend/Dockerfile` (multi-stage: target `dev` para compose, target `prod` = build Vite + Caddy estático, es el default). El Caddyfile de prod está embebido en `openshift/01-config.yaml` (ConfigMap), separado del `Caddyfile` local.
- `backend/Dockerfile` y `frontend/Dockerfile` (stage prod) están endurecidos para OpenShift: `USER 1001`, archivos con `chgrp 0`/`chmod g=u` (OpenShift corre con UID aleatorio + GID 0). Siguen funcionando en local.
...
- `openshift/` — manifiestos de despliegue en OpenShift (numerados 00→10) + su propio README con el orden y los placeholders (`REGISTRY`, `APP_HOST`, `APP_SECRET_PATH`, namespace).
